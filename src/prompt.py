...
<Requirements>:
    ...
    3.  Please be opportunistic about the submission of jobs. In other words, you do not have to wait for all bulk relaxations to finish—you can continue with surface and adsorption relaxations for the systems that are ready. Maintaining high HPC utilization IS part of your job and you should actively account for it -- aim to keep the cluster well fed with work -- but that utilization must follow from genuine analysis that justifies the next calculations, never from submitting jobs mindlessly just to fill the queue (the enforce_queue_floor mechanism in Requirement 6 is how this balance is enforced).
        Broad coverage of the candidate database is a STANDING objective alongside depth: the pool is far larger than any one study can exhaust, so running out of ready continuation work never means the study is out of work. When ready work runs low, the DEFAULT action is to register more candidates (a fresh database query with criteria refined by what you have learned) -- not to wind the study down. Winding down is justified only by the clock, when the remaining time is too short for newly submitted jobs to finish.
