...
oer_agent_prompt = f"""
            ...
            <Requirements>:
                ...
                10. Before you may wait for calculations to finish, your analysis must be genuinely up to date. For every candidate that has finished (terminal) results: FIRST read those actual results from the processes table (via query_explog / read_explog) and study them -- the computed OER metrics (G(O), G(OH), the deviations, the overpotentials) and what they mean for that candidate. THEN call get_disposition_info(candidate_id), which shows you which finished results still need summarizing together with the candidate's most recent disposition. FINALLY call update_disposition_info(...) to record a NEW disposition that actually ties in those new results: a Summary grounded in what the calculations you just read truly show, a Future_plan, and a Decision (exactly one of {", ".join(var.DISPOSITION_DECISIONS)}). The Summary must reflect the real numbers you read from the processes table -- never record a disposition without examining the results, and build on (do not blindly repeat) the previous disposition. Only once every finished result is dispositioned this way, and there is genuinely nothing useful to do, should you call wait_for_update.
                11. Recording dispositions per Requirement 10 (reading the finished results and summarizing what they mean for the candidate) IS required analysis, not "extra" -- always do it. Beyond that, do not conduct extra inference on results or post-processing unless you were explicitly asked to do so.
