...
<Requirements>:
    ...
    6.  Each plan step carries an `enforce_queue_floor` flag. Keeping it True (the default) maximizes HPC utilization, which is generally desirable: it requires the worker to keep the HPC queue stocked with ready work, and if the queue falls below the floor while HPC resources are still available and the worker has no more ready work to submit, the worker will return to YOU, the supervisor, to discuss how to expand the study (more candidates, terminations, or adsorption sites to use the free capacity) -- rather than being allowed to pause and wait. Set it False only when genuinely winding the study down -- when the remaining time is too short for newly-submitted jobs to finish -- so the worker may instead wait for and finalize the in-flight results. So when a worker returns to you blocked by the queue floor: if it reports specific READY continuation jobs waiting, you will usually want to add a plan step to submit those jobs immediately -- that keeps the cluster fed and is the right move unless you are genuinely winding the study down; otherwise (no ready work remains) either plan an expansion, or (if winding down) set enforce_queue_floor=False on its next step.
        """
...
oer_agent_prompt = f"""
            ...
            <Requirements>:
                ...
                12. If wait_for_update REFUSES because the HPC queue is under-utilized (below its floor), that is a RETURN-TO-SUPERVISOR event, NOT a retry. Do NOT re-call wait, and do NOT submit jobs outside your current task. End your turn and return to the supervisor, reporting exactly what the wait tool told you: either the specific ready jobs it listed (so the supervisor can add a plan step to submit them), or -- if it reports no ready work remains -- that the study needs expanding (new candidates / terminations / sites) or winding down.
